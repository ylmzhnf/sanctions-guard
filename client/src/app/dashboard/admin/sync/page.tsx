...
export default function DataSyncPage() {
  ...
  const syncMutation = useMutation({
    mutationFn: async () =>
      (await api.post("/admin/sanctions-sync/trigger")).data,
    onSuccess: (data) => {
      setSyncStatus({
        text: data.message || "Sync Protocol Initiated",
        type: "success",
      });
    ...
    onError: (err: any) => {
      setSyncStatus({
        text: err.response?.data?.message || "Sync Protocol Failed.",
        type: "error",
      });
      setTimeout(() => setSyncStatus({ text: "", type: null }), 6000);
    },
        ...
        <div className="flex items-start gap-6 relative z-10 w-full lg:w-auto">
          ...
          <div>
            ...
            <h1 className="text-4xl font-black text-foreground tracking-tighter uppercase italic">
              Sanctions
              <span className="text-primary not-italic bg-clip-text text-transparent bg-gradient-to-r from-primary to-blue-500">
                {" "}
                Engine
              </span>
            </h1>
            <p className="text-muted-foreground text-sm font-medium mt-2 max-w-lg leading-relaxed">
              Real-time synchronization matrix for global watchlists including
              OFAC, EU, and UN databases.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        {[
          ...
          <div
            key={i}
            className={cn(
              "bg-card/60 backdrop-blur-xl p-8 rounded-[2rem] border border-border/50 shadow-sm transition-all duration-500 relative overflow-hidden group",
              stat.border,
            )}
          >
          ...
          <div className="space-y-3 relative z-10">
            {statusLoading ? (
              ...
              statusData?.bySource?.map((s: any) => (
                ...
                >
                  <span className="bg-background px-3 py-1.5 rounded-lg border border-border/50 uppercase tracking-widest shadow-sm text-foreground">
                    {s.listSource}
                  </span>
            ...
            <tbody className="text-xs font-bold">
              {logsLoading ? (
                ...
                <tr>
                  <td
                    colSpan={6}
                    className="p-20 text-center text-muted-foreground font-black text-[10px] uppercase tracking-widest"
                  >
                    No sync logs found.
                  </td>
                </tr>
                ...
                logsData.map((log: any) => (
                  <tr
                    key={log.id}
                    className="bg-background/40 hover:bg-muted/50 transition-all group shadow-sm hover:shadow-md rounded-2xl"
                  >
                    <td className="px-6 py-5 rounded-l-2xl">
                      <span className="bg-card border border-border/50 shadow-sm px-3 py-1.5 rounded-lg text-[10px] tracking-widest uppercase text-foreground">
                        {log.source}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-5 text-center text-emerald-500 font-mono text-sm group-hover:scale-110 transition-transform">
                      +{log.recordsAdded}
                    </td>
                    <td className="px-6 py-5 text-center text-destructive font-mono text-sm group-hover:scale-110 transition-transform">
                      -{log.recordsRemoved}
                    </td>
                    <td className="px-6 py-5 text-destructive text-[10px] font-medium max-w-xs break-words">
                      {log.error || "-"}
                    </td>
                    <td className="px-6 py-5 text-right text-muted-foreground font-medium rounded-r-2xl tracking-widest uppercase text-[10px]">
