...
export default function SettingsPage() {
  ...
  const hasChanges = useMemo(() => {
    ...
    return (
      threshold !== (config.threshold ?? config.aiThreshold ?? 85) ||
      isAiEditing ||
      isOsintEditing
    );
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isAiEditing && aiKey !== config?.maskedAiKey)
        payload.aiApiKey = aiKey;
      if (isOsintEditing && osintKey !== config?.maskedOsintKey)
        payload.osintApiKey = osintKey;
  ...
  const thresholdLabel =
    threshold >= 90 ? "Precision" : threshold >= 75 ? "Balanced" : "Broad";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        <aside className="lg:col-span-4 space-y-3">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Minimum similarity score required to flag a match. Higher values
          ...
          <div className="bg-slate-50 border border-border rounded-lg p-4 space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              Recommended: 75–90% for most workflows.
            </div>
          </div>
        </aside>
        ...
        <div className="lg:col-span-8 bg-card border border-border rounded-lg p-6 md:p-8 shadow-sm">
          ...
          <div
            className={cn(
              "text-5xl md:text-6xl font-semibold tracking-tight mb-6",
              thresholdColor,
            )}
          >
            {threshold}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          <KeyInput
            ...
            value={osintKey}
            onChange={setOsintKey}
            isEditing={isOsintEditing}
            setIsEditing={setIsOsintEditing}
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          ...
          placeholder={
            hasKey
              ? "••••••••••••••••••••"
              : "Not configured — click Update to add"
          }
